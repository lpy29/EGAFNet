...
class SemSegTester(TesterBase):
    def test(self):
        ...
        for idx, data_dict in enumerate(self.test_loader):
            ...
            if os.path.isfile(pred_save_path):
                ...
                if "origin_segment" in data_dict.keys():
                    ...
                    segment = data_dict["origin_segment"]
                # vis 
                # pred_ply_save_path = os.path.join(save_path, "{}_pred.ply".format(data_name))
                # gt_ply_save_path = os.path.join(save_path, "{}_gt.ply".format(data_name))
                # write_ply(pred_ply_save_path, [data_dict["coord"], pred.astype(np.int8)], ['x', 'y', 'z', 'values'])
                # write_ply(gt_ply_save_path, [data_dict["coord"], segment], ['x', 'y', 'z', 'values'])
                # np.save(pred_save_path, pred)
                gt_all .extend(segment)  # 用来存储所有真实标签
                pred_all .extend(pred)  # 用来存储所有预测标签
            if (
                self.cfg.data.test.type == "ScanNetDataset"
                or self.cfg.data.test.type == "ScanNet200Dataset"
            ):
        ...
        import seaborn as sns  # 用于更美观的图形显示
        labels = [0,1, 2,3, 4,5,6]
        cm = confusion_matrix(gt_all, pred_all, labels=labels)
        
        cm_normalized = cm / cm.sum(axis=1)[:, np.newaxis]  # 
        
        class_names = ['Ground', 'Vegetation','Lowveg',  'Wire', 'Building',  'Light']
        
        # 计算每个类别的精度 (precision) 和召回率 (recall)
        precision = np.diagonal(cm) / np.sum(cm, axis=0)
        recall = np.diagonal(cm) / np.sum(cm, axis=1)
        
        # 在混淆矩阵下方添加一行 precision
        cm_with_precision = np.vstack([cm_normalized, precision])

        # 在混淆矩阵右侧添加一列 recall
        cm_with_precision_and_recall = np.hstack([cm_with_precision, np.append(recall, np.nan).reshape(-1, 1)])

        # 绘制热图
        # 绘制热图
        plt.rcParams['font.size'] = 12  # 设置字体大小
        plt.figure(figsize=(8, 6))
        sns.heatmap(cm_with_precision_and_recall, annot=True, fmt=".2f", cmap="Blues", cbar=False)
        ...
        #                        8: 'Buildings'}
        # 添加标签
        # class_names =  ['Ground', 'Vegetation','Cars','Trucks','Power lines','Fences','Poles', 'Building']

        # 调整刻度位置和标签数量
        xtick_labels = class_names + [''] + ['Recall']
        ytick_labels = class_names + [''] + ['Precision']

        # 设置 x 轴刻度
        plt.xticks(np.arange(len(xtick_labels)) + 0.5, xtick_labels, rotation=0)

        # 设置 y 轴刻度
        plt.yticks(np.arange(len(ytick_labels)) + 0.5, ytick_labels, rotation=0)

        # 添加分割线
        # plt.axhline(len(class_names), color='black', linewidth=1)  # 水平分割线
        # plt.axvline(len(class_names), color='black', linewidth=1)  # 垂直分割线

        plt.title("Confusion Matrix with Precision and Recall")
        plt.xlabel("Predicted")
        plt.ylabel("Ground Truth")

        plt.savefig('dales_fusion_confusion_matrix.png', dpi=300, bbox_inches='tight')
