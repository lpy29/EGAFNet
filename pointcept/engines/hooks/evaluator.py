...
@HOOKS.register_module()
class FusionSegEvaluator(HookBase):
    def after_epoch(self):
        if self.trainer.cfg.evaluate:
            self.eval()
    ...
    def eval(self):
        ...
        # test_nDSM = self.trainer.test_nDSM
        # test_proj = self.trainer.test_proj
        # test_zstd = self.trainer.test_zstd
        # test_twod_label =self.trainer.test_twod_label
        
        img_total_hist = torch.zeros((self.trainer.cfg.data.num_classes, self.trainer.cfg.data.num_classes), dtype=torch.int64, device="cuda")
        ...
        for i, input_dict in enumerate(self.trainer.val_loader):
            ...
            #     ndsm_path = os.path.join(self.trainer.cfg.data_root, "test_split_img",file_name+"_ndsm.npy")
            #     # 读入单通道的png图像
            #     img = np.load(ndsm_path)
            #     test_nDSM.append(img)
            #     std_path = os.path.join(self.trainer.cfg.data_root, "test_split_img",file_name+"_std.npy")
            #     std = np.load(std_path)
            #     test_zstd.append(std)
            #     avg_path = os.path.join(self.trainer.cfg.data_root, "test_split_img",file_name+"_avg.npy")
            #     zavg = np.load(avg_path)
            #     test_zavg.append(zavg)
            #     min_path = os.path.join(self.trainer.cfg.data_root, "test_split_img",file_name+"_min.npy")
            #     min = np.load(min_path)
            #     test_zmin.append(min)
            #     test_label_path = os.path.join(self.trainer.cfg.data_root, "test_split_img",file_name+"_gt.npy")
            #     test_label = np.load(test_label_path)
            #     test_twod_label.append(test_label)
            ...
            # with torch.no_grad():
            #     img_output = output_dict["img_logits"]
               
            #     pred, label = img_output[0].to("cuda"), torch.tensor(twod_label_i[0]).to("cuda")
            #     pred = F.softmax(pred, dim=1)
            #     pred = torch.argmax(pred, dim=1) # 假设输出是 'NCHW' 格式
            #     hist = fast_hist(label.flatten(), pred.flatten(), self.trainer.cfg.data.num_classes)
            #     img_total_hist += hist
        ...
        # iou_per_class,miou = compute_iou(img_total_hist)
        # self.trainer.logger.info(f"IoU per class: {iou_per_class.tolist()}")
        # self.trainer.logger.info(f"mIoU per class: {miou}")
